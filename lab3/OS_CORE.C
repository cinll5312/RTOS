...
void  OSInit (void)
{
    ...
    tindex = 0;          //lab1 for buf
    pindex = 0;         //lab1 for print buf
    i=0;
...
void  OSIntExit (void)
{
    ...
    if (OSRunning == TRUE) {
        ...
        if ((OSIntNesting == 0) && (OSLockNesting == 0)) { /* Reschedule only if all ISRs complete ... */
            ...
            //     // buf[index][ptcb->OSTCBPrio]=ptcb->deadline;
            ...
            if (OSPrioHighRdy != OSPrioCur) {              /* No Ctx Sw if current task is highest rdy */
                ...
                //     buf[tindex][0]=2;                           /*lab1 for preempt*/
                //     buf[tindex][1]=OSPrioCur;

                //     buf[tindex][2]=OSPrioHighRdy;                   /*lab1 [2] preempt [1]*/
                //     buf[tindex][3]=OSTime;
                //     tindex = tindex + 1;
                // }
                buf[tindex][0]=2;                           /*lab1 for preempt*/
                ...
                buf[tindex][2]=OSPrioHighRdy;                   /*lab1 [2] preempt [1]*/
                buf[tindex][3]=OSTime;
                if(OSPrioHighRdy != 1 && OSPrioHighRdy != 2)//lab3
                    tindex = tindex + 1;
            ...
            if (ptcb->OSTCBDly != 0) {                     /* Delayed or waiting for event with TO     */
                if (--ptcb->OSTCBDly == 0) {               /* Decrement nbr of ticks to end of delay   */
                    if ((ptcb->OSTCBStat & OS_STAT_SUSPEND) == OS_STAT_RDY) { /* Is task suspended?    */
                        ...
                        OSRdyTbl[ptcb->OSTCBY] |= ptcb->OSTCBBitX; /* RDY=0, ptcb is not suspension now add to ready*/
...
void  OS_Sched (void)
{
    ...
    if ((OSIntNesting == 0) && (OSLockNesting == 0)) { /* Sched. only if all ISRs done & not locked    */
        ...
        // while(ptcb->OSTCBPrio==1 || ptcb->OSTCBPrio==2 || ptcb->OSTCBPrio==3){
        //     // buf[tindex][ptcb->OSTCBPrio] = ptcb->deadline;
        //     if(ptcb->OSTCBStat==OS_STAT_RDY && !ptcb->OSTCBDly && ptcb->deadline<minDeadline){
        //         minDeadline = ptcb->deadline;
        //         OSPrioHighRdy = ptcb->OSTCBPrio;
        //     }
        ...
        // buf[tindex][0]=1;                                  /*lab1 complete task     */
        // buf[tindex][1]=OSTCBCur->OldPrio;
        // buf[tindex][2]=OSPrioHighRdy;
        // buf[tindex][3]=OSTime;
        // tindex = tindex + 1;
        ...
        if (OSPrioHighRdy != OSPrioCur) {              /* No Ctx Sw if current task is highest rdy     */
            ...
            buf[tindex][0]=1;                                  /*lab1 complete task     */
            if(OSPrioCur == 1 || OSPrioCur == 2)                //lab3
                buf[tindex][1]=OSTCBCur->OldPrio;
            else 
                buf[tindex][1]=OSPrioCur;
            buf[tindex][2]=OSPrioHighRdy;
