        ...
        if (OSIntNesting == 0) {                           /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0) {                      /* ... and not locked.                      */
                ...
                if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy */
                    ...
                    buf[tindex][0]=0;                           /*lab1 for preempt*/
                    buf[tindex][1]=OSTCBCur->OSTCBPrio;

                    buf[tindex][2]=OSTCBHighRdy->OSTCBPrio;
                    buf[tindex][3]=OSTime;
                    if(tindex < 200)tindex = tindex + 1;

                    /*lab1 [2] preempt [1]*/
                    OSCtxSwCtr++;                          /* Keep track of the number of ctx switches */
                    OSIntCtxSw();                          /* Perform interrupt level ctx switch       */
                    if(OSTime>0&&((OSTime-OSTCBPrioTbl[3]->period)> OSTCBPrioTbl[3]->start_time)){
...
void  OSStart (void)
{
    ...
    if (OSRunning == OS_FALSE) {
        ...
        buf[tindex][0]=0;                                  /*lab1 complete task     */
